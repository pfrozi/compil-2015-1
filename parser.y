...
statement:
          TK_PR_STATIC type TK_IDENTIFICADOR	    { if(yyfind_stack($3)!=NULL) return IKS_ERROR_DECLARED; }
        | type TK_IDENTIFICADOR			    { if(yyfind_stack($2)!=NULL) return IKS_ERROR_DECLARED; }
...
local_statement:
          const_statement TK_OC_LE TK_IDENTIFICADOR	{ if(yyfind_stack($3)!=NULL) return IKS_ERROR_DECLARED; }
        | const_statement TK_OC_LE literal
        | statement TK_OC_LE TK_IDENTIFICADOR		{ if(yyfind_stack($3)!=NULL) return IKS_ERROR_DECLARED; }
...
;
const_statement:
          TK_PR_STATIC TK_PR_CONST type TK_IDENTIFICADOR	{ if(yyfind_stack($4)!=NULL) return IKS_ERROR_DECLARED; }
        ...
        | TK_PR_CONST type TK_IDENTIFICADOR			{ if(yyfind_stack($3)!=NULL) return IKS_ERROR_DECLARED; }
...
exp_end:
          func_call                              { $$ = $1; }
        | op_literal                             { $$ = $1; }
        | TK_IDENTIFICADOR                       { if(yyfind_stack($1)==NULL) return IKS_ERROR_UNDECLARED; $$ = cc_tree_create_node(1,cc_tree_item_create(AST_IDENTIFICADOR,$1)); }
        | TK_IDENTIFICADOR array                 { if(yyfind_stack($1)==NULL) return IKS_ERROR_UNDECLARED; $$ = cc_tree_insert_node(cc_tree_insert_node(cc_tree_create_node(2,cc_tree_item_create(AST_VETOR_INDEXADO,NULL)),cc_tree_create_node(1,cc_tree_item_create(AST_IDENTIFICADOR,$1))),$2);}
;
...
;
func_head:
          TK_PR_STATIC type TK_IDENTIFICADOR func_head_params  { if(yyfind_stack($3)!=NULL) return IKS_ERROR_DECLARED; $$ = cc_tree_create_node(3,cc_tree_item_create(AST_FUNCAO,$3));}
        | type TK_IDENTIFICADOR func_head_params               { if(yyfind_stack($3)!=NULL) return IKS_ERROR_DECLARED; $$ = cc_tree_create_node(3,cc_tree_item_create(AST_FUNCAO,$2));}
...
assignment:
        ...
        | TK_IDENTIFICADOR array TK_CE_EQUAL exp    { if(yyfind_stack($1)==NULL) return IKS_ERROR_UNDECLARED; $$ = cc_tree_insert_node(cc_tree_insert_node(cc_tree_create_node(3,cc_tree_item_create(AST_ATRIBUICAO,NULL)),cc_tree_insert_node(cc_tree_insert_node(cc_tree_create_node(2,cc_tree_item_create(AST_VETOR_INDEXADO,NULL)),cc_tree_create_node(1,cc_tree_item_create(AST_IDENTIFICADOR,$1))),$2)),$4);}
...
// Chamada da funcao 
func_call:
          TK_IDENTIFICADOR TK_CE_PAR_OPEN TK_CE_PAR_CLOSE { if(yyfind_stack($1)!=NULL) return IKS_ERROR_VARIABLE; $$ = cc_tree_insert_node(cc_tree_create_node(2,cc_tree_item_create(AST_CHAMADA_DE_FUNCAO,NULL)),cc_tree_create_node(1,cc_tree_item_create(AST_IDENTIFICADOR,$1)));}
        | TK_IDENTIFICADOR TK_CE_PAR_OPEN lst_exp TK_CE_PAR_CLOSE { if(yyfind_stack($1)!=NULL) return IKS_ERROR_VARIABLE; $$ = cc_tree_insert_node(cc_tree_insert_node(cc_tree_create_node(3,cc_tree_item_create(AST_CHAMADA_DE_FUNCAO,NULL)),$3),cc_tree_create_node(1,cc_tree_item_create(AST_IDENTIFICADOR,$1)));}
;
// if
