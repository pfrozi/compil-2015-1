...
programa:
          input TOKEN_EOF {
                              $$ = cc_tree_insert_node(cc_tree_create_node(1,cc_tree_item_create(AST_PROGRAMA,NULL)),$1); 
                              ast=$$; 
...
func_call:
        ...
        | TK_IDENTIFICADOR TK_CE_PAR_OPEN lst_exp TK_CE_PAR_CLOSE { if(yystack_find($1)==NULL) return IKS_ERROR_UNDECLARED; if(yystack_find($1)->iks_var!=IKS_FUNCTION) return yyerror_var($1); comp_list_t* args = yytree_get_argument_list($3); if(cc_list_length($1->iks_arguments)<cc_list_length(args)) return IKS_ERROR_EXCESS_ARGS; if(cc_list_length($1->iks_arguments)>cc_list_length(args)) return IKS_ERROR_MISSING_ARGS; if(yytree_match_arguments($1->iks_arguments,args)>0) return IKS_ERROR_WRONG_TYPE_ARGS; $$ = cc_tree_insert_node(cc_tree_insert_node(cc_tree_create_node(3,cc_tree_item_create_type(AST_CHAMADA_DE_FUNCAO,yystack_find($1)->iks_type,NULL)),cc_tree_create_node(1,cc_tree_item_create_type(AST_IDENTIFICADOR,yystack_find($1)->iks_type,$1))),$3);}
;
// if
